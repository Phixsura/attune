...
// Login handles POST /install/login. The response never distinguishes
// "unknown email" from "wrong password" (dummy bcrypt + generic 401);
// failure counts are tracked on the admin row so a real attacker hits
// the 5-strike / 15-minute lockout, but enumeration via timing is not
// possible.
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	a, err := h.admins.GetByEmail(ctx, body.Email)
	switch {
	case errors.Is(err, admin.ErrNotFound):
		// Equalise timing with a dummy bcrypt run; result discarded.
		_ = VerifyOrDummy("", body.Password)
	...
	if !VerifyOrDummy(a.PasswordHash, body.Password) {
	...
	redirect := "/console/"
	if body.RedirectURI != "" && redirectIsSafe(h.baseURL, body.RedirectURI) {
