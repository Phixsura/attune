...
func (h *FeedbackHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respond.Error(ctx, w, http.StatusInternalServerError, "internal", "查询反馈失败")
		return
	}
...
func (h *FeedbackHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		respond.Error(ctx, w, http.StatusBadRequest, "bad_id", "id 必须是整数")
		return
	}
	...
	if err != nil {
		if errors.Is(err, feedback.ErrFeedbackNotFound) {
			...
			respond.Error(ctx, w, http.StatusNotFound, "not_found", "反馈不存在或不属于当前 tenant")
			return
		}
		...
		respond.Error(ctx, w, http.StatusInternalServerError, "internal", "加载反馈详情失败")
		return
	}
...
func (h *FeedbackHandler) Stats(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		logext.Errorf(ctx, "[%s] feedback.KindCounts failed,tenant_id:%s,err:%+v",
			where, auth.TenantID, err.Error())
		respond.Error(ctx, w, http.StatusInternalServerError, "internal", "查询反馈分布失败")
		return
	}
	var total int64
