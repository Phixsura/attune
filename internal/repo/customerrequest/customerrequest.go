...
// FeedbackSourceMeta is the promote-time attribution read: the row's
// channel, metadata, and its GDPR subject identity.
type FeedbackSourceMeta struct {
	Source     string
	Meta       map[string]any
	SubjectKey string
}

func (r *Repo) FeedbackSourceMetaTx(ctx context.Context, tx pgx.Tx, tenantID string, feedbackID int64) (FeedbackSourceMeta, error) {
	var out FeedbackSourceMeta
	...
	err := tx.QueryRow(
		ctx, `
		SELECT source, source_meta, subject_key
		FROM user_feedback
		WHERE tenant_id = $1 AND id = $2 AND deleted_at IS NULL
		FOR SHARE`,
		tenantID, feedbackID,
	).Scan(&out.Source, &out.Meta, &out.SubjectKey) // ptrext:allow pgx-scan
	if errors.Is(err, pgx.ErrNoRows) {
		return FeedbackSourceMeta{}, ErrFeedbackNotFound
	}
	if err != nil {
		return FeedbackSourceMeta{}, fmt.Errorf("feedback source meta: %w", err)
	}
	return out, nil
}
...
func (r *Repo) BindIssueExternalObjectLinkTx(
	ctx context.Context,
	tx pgx.Tx,
	tenantID string,
	requestID uuid.UUID,
	...
	tag, err := tx.Exec(ctx, `
		UPDATE customer_request_issue_links
		 ...
		 WHERE tenant_id = $1
		   AND request_id = $2
		   AND id = $3`,
		tenantID, requestID, issueLinkID, externalObjectLinkID)
	if err != nil {
		return nil, fmt.Errorf("bind customer request issue external object link: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return nil, ErrLinkNotFound
	}
	return getIssueLink(ctx, tx, tenantID, requestID, issueLinkID)
}
