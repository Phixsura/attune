...
func TestRepoUpsertStoresAndListsViews(t *testing.T) {
	...
	first, err := repo.Upsert(t.Context(), "tenant-1", "user-1", View{
		Name: "High-value backlog",
		State: State{
			Query:      " enterprise ",
			Statuses:   []crrepo.Status{crrepo.StatusOpen, crrepo.StatusOpen, crrepo.StatusPlanned},
			Priorities: []crrepo.Priority{crrepo.PriorityHigh, crrepo.PriorityUrgent},
	...
	second, err := repo.Upsert(t.Context(), "tenant-1", "user-1", View{
		Name:  "Delivery risk",
		State: State{Sort: crrepo.SortDeliveryHealth},
	}, "user-1")
	if err != nil {
	...
	if listed[0].Name != "Delivery risk" {
		t.Fatalf("List() order = %#v, want latest first", listed)
	}
	if listed[1].State.Query != "enterprise" || len(listed[1].State.Statuses) != 2 {
...
func TestRepoUpsertUpdatePreservesMetadataAndDetectsConflict(t *testing.T) {
	...
	first, err := repo.Upsert(t.Context(), "tenant-1", "user-1", View{
		Name:  "First view",
		State: State{Sort: crrepo.SortUpdatedAt},
	}, "user-1")
	if err != nil {
		t.Fatalf("Upsert(first) err = %v", err)
	}
	updated, err := repo.Upsert(t.Context(), "tenant-1", "user-1", View{
		...
		State: State{Sort: crrepo.SortRevenueImpact},
	}, "user-1")
	if err != nil {
