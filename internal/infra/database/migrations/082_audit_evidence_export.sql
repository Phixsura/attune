-- Async audit evidence export jobs (#152).
--
-- Stores tamper-evident evidence pack generation jobs with claim/heartbeat
-- worker semantics, modeled on gdpr_export_jobs (migration 039).

CREATE TABLE IF NOT EXISTS audit_evidence_export (
    ...
    CONSTRAINT chk_audit_evidence_export_status CHECK (
        status IN ('queued', 'running', 'completed', 'failed', 'expired')
    ),
    CONSTRAINT chk_audit_evidence_export_total_events CHECK (total_events >= 0)
);

CREATE INDEX IF NOT EXISTS idx_audit_evidence_export_tenant_created
    ON audit_evidence_export (tenant_id, created_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_audit_evidence_export_status_created
    ON audit_evidence_export (status, created_at ASC)
    WHERE status = 'queued';

CREATE INDEX IF NOT EXISTS idx_audit_evidence_export_expiry
    ON audit_evidence_export (expires_at)
    WHERE status = 'completed' AND expires_at IS NOT NULL;

-- Extend audit_log action constraint with evidence export actions.
