-- Add explicit GDPR export revocation state and audit action (#43 hardening).

ALTER TABLE gdpr_export_jobs
    ADD COLUMN IF NOT EXISTS revoked_at TIMESTAMPTZ;

ALTER TABLE gdpr_export_jobs
    DROP CONSTRAINT IF EXISTS chk_gdpr_export_jobs_status;

ALTER TABLE gdpr_export_jobs
    ADD CONSTRAINT chk_gdpr_export_jobs_status CHECK (
...
ALTER TABLE gdpr_requests
    ADD CONSTRAINT chk_gdpr_requests_status CHECK (
        status IN ('queued', 'running', 'ready', 'downloaded', 'completed', 'failed', 'expired', 'scheduled', 'cancelled', 'revoked')
    );
