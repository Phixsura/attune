...
--
-- Public request comments and audit allow-list.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE TABLE IF NOT EXISTS customer_request_comments (
    ...
    CONSTRAINT fk_customer_request_comments_request
        FOREIGN KEY (tenant_id, request_id)
        REFERENCES customer_requests(tenant_id, id)
        ON DELETE CASCADE,
    CONSTRAINT chk_customer_request_comments_body_length CHECK (length(body) BETWEEN 1 AND 5000),
    CONSTRAINT chk_customer_request_comments_identity
        CHECK (subject_key <> '' OR subject_hash <> ''),
    CONSTRAINT chk_customer_request_comments_subject_key_length CHECK (length(subject_key) <= 512),
    CONSTRAINT chk_customer_request_comments_subject_hash_length CHECK (length(subject_hash) <= 128),
    CONSTRAINT chk_customer_request_comments_subject_display_length CHECK (length(subject_display) <= 500)
);

CREATE INDEX IF NOT EXISTS idx_customer_request_comments_tenant_request
    ON customer_request_comments (tenant_id, request_id, created_at, id);
CREATE INDEX IF NOT EXISTS idx_customer_request_comments_identity
    ON customer_request_comments (tenant_id, subject_hash, subject_key);
