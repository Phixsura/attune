-- Classification review learning ledger.
--
-- Human review of AI classification samples is stored as append-only evidence.
-- The current classifier remains the fast serving snapshot; this table keeps
-- accepted, corrected, and dismissed examples for evaluation and future tuning.
...
CREATE TABLE IF NOT EXISTS classification_review_events (
    id                        BIGSERIAL PRIMARY KEY,
    tenant_id                 TEXT NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
    feedback_id               BIGINT NOT NULL REFERENCES user_feedback(id) ON DELETE CASCADE,
    semantic_run_id           BIGINT REFERENCES semantic_extraction_runs(id) ON DELETE SET NULL,
    ...
    CHECK (outcome IN ('accepted', 'edited', 'dismissed')),
    CHECK (length(signal_reason) <= 120),
    CHECK (length(note) <= 1000),
    CHECK (length(source) <= 120),
    CHECK (length(logical_model) <= 120),
    CHECK (length(provider_model) <= 120),
    CHECK (length(channel_id) <= 120),
    CHECK (length(prompt_version) <= 120),
    CHECK (length(prompt_version_id) <= 80),
    CHECK (
...
CREATE INDEX IF NOT EXISTS idx_classification_review_events_window
    ON classification_review_events (tenant_id, reviewed_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_classification_review_events_feedback
    ON classification_review_events (tenant_id, feedback_id, reviewed_at DESC);

CREATE INDEX IF NOT EXISTS idx_classification_review_events_outcome
    ON classification_review_events (tenant_id, outcome, reviewed_at DESC);
