...
// CustomWebhookDest is one row's worth of raw-webhook config. It is
// upserted into tenant_notify_targets at startup; everything in the
...
func (c *Config) validateCustomWebhooks() error {
	...
	for i, w := range c.CustomWebhooks {
		...
		key := w.TenantSlug + "|" + audience
		if seen[key] {
			return fmt.Errorf("custom_webhooks[%d]: duplicate (tenant_slug=%q, audience=%q)",
				i, w.TenantSlug, audience)
		}
		seen[key] = true
	}
	return nil
