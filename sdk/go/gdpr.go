...
// Re-exported GDPR wire types (generated). GDPR jobs need a key with the
// `gdpr:admin` scope.
type (
...
// ExportGdprSubject starts a GDPR export job (needs `gdpr:admin`).
func (c *Client) ExportGdprSubject(ctx context.Context, req *ExportGdprSubjectRequest) (*ExportGdprSubjectResponse, error) {
	...
	if err := c.do(ctx, http.MethodPost, "/v1/gdpr/export", payload, &out, ""); err != nil {
		return nil, err
	}
	return &out, nil
}

// GetGdprExport returns one GDPR export job by id (needs `gdpr:admin`).
...
// RevokeGdprExport revokes one GDPR export job by id (needs `gdpr:admin`).
func (c *Client) RevokeGdprExport(ctx context.Context, jobID string) (*RevokeGdprExportResponse, error) {
	if !validPathSegment(jobID) {
		return nil, &AttuneError{Code: CodeBadRequest, Message: "gdpr export job id is invalid"}
	}
	var out attunev1.RevokeGdprExportResponse
	if err := c.do(ctx, http.MethodPost, "/v1/gdpr/exports/"+url.PathEscape(jobID)+"/revoke", nil, &out, ""); err != nil {
		return nil, err
	}
	return &out, nil
}

// DeleteGdprSubject starts a GDPR delete request (needs `gdpr:admin`).
func (c *Client) DeleteGdprSubject(ctx context.Context, req *DeleteGdprSubjectRequest) (*DeleteGdprSubjectResponse, error) {
	...
	if err := c.do(ctx, http.MethodPost, "/v1/gdpr/delete", payload, &out, ""); err != nil {
		return nil, err
	}
	return &out, nil
}

// CancelGdprRequest cancels one GDPR request by id (needs `gdpr:admin`).
func (c *Client) CancelGdprRequest(ctx context.Context, requestID string) (*CancelGdprRequestResponse, error) {
	...
	if err := c.do(ctx, http.MethodPost, "/v1/gdpr/requests/"+url.PathEscape(requestID)+"/cancel", nil, &out, ""); err != nil {
		return nil, err
	}
	return &out, nil
}
...
// `gdpr:admin`). Pass nil for the default query.
...
// GetGdprOperations returns GDPR operational metadata (needs `gdpr:admin`).
