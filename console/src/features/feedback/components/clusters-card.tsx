...
export function ClustersCard() {
  ...
  if (clusters.isPending) {
    return (
      <Card className="gap-4 rounded-[0.95rem] border-border/40 bg-background py-3 shadow-none">
          ...
          <div className="space-y-2">
            {items.slice(0, 3).map((cluster) => (
              <Link
                ...
                to="/feedback/clusters"
                className="flex w-full items-center gap-3 rounded-md border border-border/50 bg-muted/6 px-3 py-2.5 text-left transition-colors hover:bg-muted/20"
              >
