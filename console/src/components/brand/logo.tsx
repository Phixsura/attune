...
// Attune brand mark. Three concentric sound-wave arcs paired with the
// Attune wordmark. Color inherits from currentColor so the same SVG works
// for primary (orange in light mode) and any accent context (light text
// in dark mode, brand color on white pages).
//
// Layer 1 visual polish — replaces the bare "Attune" text in TopBar.
...
export function Logo({
  ...
  return (
    <div className={cn('inline-flex items-center gap-2 leading-none', className)}>
      ...
      {showText && <span className="text-base font-semibold tracking-tight">Attune</span>}
    </div>
  )
}
