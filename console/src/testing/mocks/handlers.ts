...
export const sampleNpsSurveyCampaign: SurveyCampaign = {
  ...sampleSurveyCampaign,
  id: 'nps-campaign-1',
  name: 'Relationship NPS',
  surveyType: SurveyType.SURVEY_TYPE_NPS,
  triggerEvent: SurveyTriggerEvent.SURVEY_TRIGGER_EVENT_SCHEDULED_RUN,
  distributionMode: SurveyDistributionMode.SURVEY_DISTRIBUTION_MODE_CONTACT_EMAIL,
  dedupePolicy: SurveyDedupePolicy.SURVEY_DEDUPE_POLICY_ONE_PER_RUN,
  triggerFilter: {},
  content: {
...
export const sampleNpsCampaignRun: NpsCampaignRun = {
  id: 'nps-run-1',
  ...
  openedAt: '2026-07-30T00:01:00Z',
  closesAt: '2026-08-13T00:01:00Z',
  evaluatedCount: 120,
  eligibleCount: 96,
  invitationCount: 96,
  deliveredCount: 91,
  ...
  npsAvailable: true,
  responseRate: 54 / 96,
  hostedVisitRate: 54 / 96,
  completionRate: 42 / 54,
  nps: 24,
  detractorCount: 9,
  passiveCount: 14,
  promoterCount: 19,
  redactedResponseCount: 0,
  measurementKey: 'nps:v2:current-definition',
  collectionDays: 14,
  maximumRunRecipients: 500,
  contactCooldownDays: 90,
  recurrenceSamplingPercent: 25,
  completedResponseRate: 42 / 96,
  measurementReadiness: NpsMeasurementReadiness.NPS_MEASUREMENT_READINESS_PRELIMINARY,
  minimumCompletedResponses: 30,
  minimumResponseRatePercent: 10,
  samplePlanningPopulationCount: 96,
...
export const sampleNpsCampaignPreflight: NpsCampaignPreflight = {
  campaignId: sampleNpsSurveyCampaign.id,
  evaluatedCount: 120,
  eligibleCount: 96,
  excludedCount: 24,
  exclusionReasonDistribution: [
    { reason: 'contact_missing', count: 9 },
    { reason: 'contact_unavailable', count: 6 },
    { reason: 'contact_cooldown', count: 9 },
  ],
  plannedInvitationCount: 96,
  maximumRunRecipients: 500,
  minimumCompletedResponses: 30,
  samplePlanningPopulationCount: 96,
  samplePlanningRequiredCompletedResponses: 49,
  ...
  samplePlanningMarginOfErrorPercent: 10,
  samplePlanningExpectedResponseRatePercent: 20,
  samplePlanningTargetExceedsRecipientCap: false,
  recurrenceSamplingPercent: 25,
  plannedInvitationCountBelowMinimumCompletedResponses: false,
  plannedInvitationCountBelowSamplePlanningTarget: true,
  deliveryReady: true,
  deliveryBlocker: '',
...
export const sampleSurveyResponse: SurveyResponse = {
  ...
  locale: 'zh-CN',
  lowScore: true,
  npsBucket: NpsBucket.NPS_BUCKET_UNSPECIFIED,
...
export const handlers = [
  ...
  http.get(`${BASE}/cohort-sync/cohorts`, () => HttpResponse.json({ cohorts: [] })),
  ...
  http.post(`${BASE}/surveys/campaigns/:id\\:scheduleNpsRun`, async ({ params, request }) => {
    const body = (await request.json()) as { clientRequestKey?: string; scheduledAt?: string }
    return HttpResponse.json(
      {
        ...sampleNpsCampaignRun,
        campaignId: String(params.id),
        scheduledAt: body.scheduledAt ?? sampleNpsCampaignRun.scheduledAt,
      },
      { status: 201 },
    )
  }),
  http.post(`${BASE}/surveys/campaigns/:campaignId/nps-runs/:runId\\:cancel`, ({ params }) =>
    HttpResponse.json({
      ...sampleNpsCampaignRun,
  ...
  http.get(`${BASE}/surveys/campaigns/:id/nps-runs`, ({ params }) =>
    HttpResponse.json({
      runs: String(params.id) === sampleNpsSurveyCampaign.id ? [sampleNpsCampaignRun] : [],
    }),
  ),
  http.post(
    `${BASE}/surveys/campaigns/:campaignId/nps-runs/:runId/evidence-exports`,
    ({ params }) =>
      HttpResponse.json(
        {
          id: 'nps-evidence-export-1',
          campaignId: String(params.campaignId),
          runId: String(params.runId),
          reportVersion: '1',
          generatedAt: '2026-08-08T01:02:03Z',
          artifactSha256: 'sha256:nps-evidence-export-1',
          createdByType: 'admin',
          createdAt: '2026-08-08T01:02:03Z',
          downloadPath: `${BASE}/surveys/campaigns/${String(params.campaignId)}/nps-runs/${String(params.runId)}/evidence-exports/nps-evidence-export-1.csv`,
          expiresAt: '2026-09-07T01:02:03Z',
        },
        { status: 201 },
      ),
  ),
  http.get(`${BASE}/surveys/campaigns/:campaignId/nps-runs/:runId/evidence-exports`, () =>
    HttpResponse.json({ exports: [] }),
  ),
  http.get(
    `${BASE}/surveys/campaigns/:campaignId/nps-runs/:runId/evidence-exports/:exportId.csv`,
    () =>
      new HttpResponse('report_version,run_id\n1,nps-run-1\n', {
        headers: {
          'Content-Type': 'text/csv; charset=utf-8',
          'Content-Disposition': 'attachment; filename="nps-run-evidence.csv"',
        },
      }),
  ),
  http.get(`${BASE}/surveys/campaigns/:id/nps-preflight`, ({ params }) =>
    HttpResponse.json({
