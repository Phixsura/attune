...
func TestMigrations_MigrationCount(t *testing.T) {
	...
	require.Equal(t, 90, count, "should have 90 migrations")
...
func TestMigrations_LoadMigrationNames_All(t *testing.T) {
	...
	// Should have 90 migrations.
	require.Len(t, names, 90)

	// First and last
	require.Equal(t, "001_init.sql", names[0])
	require.Equal(t, "090_audit_evidence_created_by_type.sql", names[len(names)-1])
