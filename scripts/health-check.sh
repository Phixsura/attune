#!/usr/bin/env bash
# listen 日常健康检查 — 一行命令看全栈。
#
# 用法 A · 本地 / 任何能联网的机器：
#     bash listen/scripts/health-check.sh
#   只验对外暴露面（nginx 反代到的所有路径）。
#
# 用法 B · 跑在 prod 上（SSH 进去后）：
#     ssh root@47.107.48.110 'bash -s' < listen/scripts/health-check.sh
...
#   :8088 (casceneai-fe nginx)   /fb/v1/...   → listen :8090 (ingest 路径)
#   :8091 (listen-console nginx) /console/    → SPA dist
#   :8091 (listen-console nginx) /fb/v1/...   → listen :8090 (console API)
# Prod-only surface = listen container loopback only.
#   :8090 /health, /metrics
...
echo "==> listen health check · mode=${MODE} · $(date '+%F %T')"
echo
...
if [[ "$MODE" == "prod" ]]; then
  check "listen /health (loopback)" bash -c "
    code=\$(curl -sS -o /dev/null -w '%{http_code}' --max-time 5 ${PROD_BASE}/health)
    [[ \$code == '200' ]] && echo 'HTTP 200' || { echo \"HTTP \$code\"; exit 1; }
  "

  check "outbox lag (< 300s)" bash -c "
    lag=\$(curl -sS --max-time 5 ${PROD_BASE}/metrics | awk '/^listen_outbox_lag_seconds /{print \$2; exit}')
  ...
  check "container Up" bash -c "
    docker ps --filter name=casceneai-listen --format '{{.Status}}' | head -1 | grep -q '^Up' && echo OK || { echo down; exit 1; }
  "
fi

echo
if [[ $FAILED -eq 0 ]]; then
  echo "${GREEN}全绿 (${PASSED} 项)。listen 是 solid 的。${RESET}"
else
  echo "${RED}${FAILED} 项红线 / ${PASSED} 项过。${RESET}"
  echo "${DIM}  排查：ssh root@${PROD_IP} 'docker logs --tail 50 casceneai-casceneai-listen-1'${RESET}"
fi
exit "$FAILED"
