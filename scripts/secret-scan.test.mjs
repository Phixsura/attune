...
test('secret-scan ignores unsafe checkout and staged paths', async () => {
  const binDir = await mkdtemp(path.join(os.tmpdir(), 'attune-secret-scan-bin-'))
  const argsLog = path.join(binDir, 'args.log')
  const changedPaths = path.join(binDir, 'changed-paths')
  const stagedPaths = path.join(binDir, 'staged-paths')
  await writeFile(changedPaths, '../outside\n/absolute\nscripts/../escape\n')
  await writeFile(stagedPaths, '../staged\n/absolute-staged\nscripts/../staged-escape\n')
  await writeExecutable(
    path.join(binDir, 'trufflehog'),
    `#!/bin/sh
{
...
test('secret-scan uses custom Docker image and results mode for snapshots', async () => {
  const binDir = await mkdtemp(path.join(os.tmpdir(), 'attune-secret-scan-bin-'))
  const argsLog = path.join(binDir, 'args.log')
  const changedPaths = path.join(binDir, 'changed-paths')
  const stagedPaths = path.join(binDir, 'staged-paths')
...
`,
  ...
  const result = runSecretScan(withSystemPath(binDir), {
    SECRET_SCAN_ARGS_LOG: argsLog,
    TRUFFLEHOG_CHANGED_PATHS_FILE: changedPaths,
    TRUFFLEHOG_IMAGE: 'example/trufflehog:pinned',
    TRUFFLEHOG_RESULTS: 'verified',
    TRUFFLEHOG_STAGED_PATHS_FILE: stagedPaths,
  })
  ...
  const calls = await readCalls(argsLog)
  assert.equal(calls.length, 2)
  assert.equal(calls[0][4], 'example/trufflehog:pinned')
  assert.equal(calls[0][6], '--results=verified')
  assert.equal(calls[1][6], 'example/trufflehog:pinned')
  assert.deepEqual(calls[1].slice(7, 10), [
    'filesystem',
    '--results=verified',
    '--exclude-paths=/repo/.trufflehogignore',
  ])
